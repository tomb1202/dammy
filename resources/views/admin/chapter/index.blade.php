...
@section('title')
    <title>Admin | Chương truyện</title>
@endsection
@section('content')
<style>
...
<section class="content-header">
    <ol class="breadcrumb">
        ...
        <li class="active">Chương truyện</li>
    </ol>
    <ul class="right-button">
        <li><a href="{{ route('admin.chapter.edit') }}" class="btn btn-success btn-sm"><i class="fa fa-plus"></i> Thêm mới</a></li>
            ...
            <div class="search mb-3">
                <form action="" method="GET">
                    <div class="row">
                        ...
                        <div class="col-sm-4">
                            <select id="comic_filter" name="comic_id" class="form-control select2-ajax" data-placeholder="Chọn truyện..." data-ajax-url="{{ route('admin.comic.search') }}">
                                @if ($request->filled('comic_id') && $selectedComic = \App\Models\Comic::find($request->comic_id))
                                    <option value="{{ $selectedComic->id }}" selected>{{ $selectedComic->title }}</option>
                                @endif
                            </select>
                        </div>
                        <div class="col-sm-4">
                            <input name="search" value="{{ $request->search ?? '' }}" class="form-control" placeholder="Tìm theo tiêu đề chương...">
                        </div>
                ...
                <tbody>
                    @foreach ($data as $index => $item)
                        <tr class="tr-{{ $item->id }}">
                            ...
                            <td>
                                <strong><a href="{{route('admin.comic.edit', ['comic' => $item->comic->id])}}">{{ $item->comic->title ?? '---' }}</a></strong><br>
                            ...
                            <td>
                                <a href="{{ route('admin.chapter.edit', ['chapter' => $item->id]) }}" class="btn btn-primary btn-sm"><i class="fa fa-pencil"></i></a>
...
<script>
    $(document).on('click', '.btn-delete', function() {
        ...
        $.ajax({
            url: "{{ route('admin.chapter.delete') }}",
            type: "DELETE",
    ...
    $(document).ready(function() {
        ...
        $('.btn-delete-multiple').on('click', function() {
            ...
            $.ajax({
                url: "{{ route('admin.chapter.delete-multiple') }}",
                type: "DELETE",
        ...
        // Init select2 AJAX
        $('#comic_filter').select2({
            placeholder: $(this).data('placeholder'),
            ajax: {
                url: $('#comic_filter').data('ajax-url'),
                dataType: 'json',
                delay: 250,
